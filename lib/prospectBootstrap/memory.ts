...
type ApprovedCompilableFact = CompilableFact & { status: "operator_approved_for_demo" | "owner_confirmed" };
...
function isApprovedFact(fact: CompilableFact): fact is ApprovedCompilableFact {
  return fact.status === "operator_approved_for_demo" || fact.status === "owner_confirmed";
}

function linkedSourceIds(fact: CompilableFact, sourceById: Map<string, CompilableSource>): string[] {
  return Array.isArray(fact.source_ids_json)
...
function selectApprovedFacts<T extends CompilableFact>(facts: T[]): T[] {
...
export function compileBusinessMemorySnapshot(params: {
  ...
  // Rank only facts whose source is usable. Ranking first would let a
  // higher-authority fact with an unacquired source win and then be skipped,
  // silently dropping the sourced fact for the same key.
  const sourceableFacts = params.facts.filter(isApprovedFact).filter((fact) => {
    const source = sourceById.get(linkedSourceIds(fact, sourceById)[0]);
    return Boolean(source?.content_hash && source.fetched_at);
  });

  for (const fact of selectApprovedFacts(sourceableFacts)) {
    const sourceIds = linkedSourceIds(fact, sourceById);
