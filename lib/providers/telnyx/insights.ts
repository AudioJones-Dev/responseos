...
export interface TelnyxCallInsight {
  canonicalInteraction: string | null;
  product: string | null;
  outcome: string | null;
  urgency: string | null;
...
function insightFrom(source: Record<string, unknown>): TelnyxCallInsight {
  ...
  return {
    ...
    canonicalInteraction: text(source.interaction_type),
    product: text(source.product_path),
    outcome: text(source.outcome),
    urgency: text(source.urgency),
    callbackWindow: text(source.callback_window),
...
function merge(base: TelnyxCallInsight, next: TelnyxCallInsight): TelnyxCallInsight {
  return {
    canonicalInteraction: base.canonicalInteraction ?? next.canonicalInteraction,
    product: base.product ?? next.product,
    outcome: base.outcome ?? next.outcome,
    urgency: base.urgency ?? next.urgency,
    callbackWindow: base.callbackWindow ?? next.callbackWindow,
