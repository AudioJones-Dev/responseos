...
| Telnyx Call Control qualification _(repository implementation; deployment and routing separately gated, ADR-0057)_ | `telnyx-signature-ed25519` + `telnyx-timestamp` | Accept strict `application/json` only; verify the raw body and five-minute freshness before parsing, allowlist event shapes, bind consequential callbacks to the pinned capture generation, and reject TeXML forms and protected content outside the qualified consent interval |
