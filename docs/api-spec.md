...
Ratified CRM recovery amendment (implementation in progress): the existing review POST adds `crm_inspect`, `crm_adopt`, and `crm_abandon`. Mutating reconciliation requires the displayed generation, a reason and evidence; adoption additionally requires the operator's explicit confirmation that the prior worker cannot resume and a fresh verified provider readback. All actions derive account/call from the authenticated operator's located review. Conflicting, not-observed or unavailable evidence leaves CRM reconciliation-required. No force-retry action is exposed. Cancellation is local only. Responses expose safe effect/binding/ID evidence, never credentials or provider payloads. CRM non-success returns `crm_incomplete` before any new email; accepted-email history is preserved.
