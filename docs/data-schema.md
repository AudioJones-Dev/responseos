...
| `consent_records` | Consent evidence for recording, AI handling and marketing. Per **ADR-0055** the record of authority is an **immutable event** — the disclosure presented, the grant or withdrawal, the timestamp, the jurisdiction basis, and the source interaction and channel (call, SMS, web form, other); append-only, so a withdrawal is a new event rather than a mutation. For a recording or persisted transcript the event must also be linked to that call and artifact class. Per-contact state is a **derived summary** of that stream for routing and preference, never the evidence relied on to justify a retained artifact. Recording and transcript persistence are evaluated independently. Absence of an event is refusal. **Not implemented** — absent from all Prisma models; see the readiness contract |
...
Codes are `crm_ready:<effect>`, `crm_intent:<effect>`, `crm_reconcile:<effect>` and `crm_legacy_unknown`. Effects advance in order: `contact_create`, `activity_create`, `activity_associate`, qualified-only `task_create`, `task_associate`, then `done`. Only `ready` is automatically claimable; `intent` becomes `review_required` on an unknown outcome or stale lease. `last_error_redacted` holds `{v:1, reviewId, payloadHash, destination}`. `ready:done` plus all required IDs supports durable success; association acknowledgment is represented by phase advancement, not inferred from the presence of an object ID. No AuditLog query is required to make these decisions.

Additive candidate runtime fields are followed by CallCaptureSession (pinned approved snapshot per tenant/provider call), CallConsentEvent (append-only per-call artifact authority carrying the ADR-0055 minimum record: disclosure, action, timestamp, jurisdiction basis, and a server-owned source channel), and CallReview (immutable evidence and approved payload revision with independent CRM/email state). New supervised calls carry review_required; legacy calls default false. See [implementation brief](./product/responseos-mike-live-demo.md). These repository migrations remain deployment-gated and do not claim live activation.

Migration 0016 adds `qualification` to `TelephonyNumberAssignmentStatus`. It is a temporary, exclusive supervised assignment with `assigned_at` and a nullable `unassigned_at`; it never receives `activated_at`. The status remains `qualification` after closure so delayed events can resolve the historical interval and business-effect authorization can permanently reject calls captured inside it. `number_exclusivity_key` is cleared on closure. Existing rows and assignment states are unchanged.
