...
| **Supervised tenant runtime** | A customer tenant resolved from its own number; typed operating configuration and one operator write path; recording as a tenant configuration rather than a tier default; webhook-ledger correlation for post-call events that carry no called number; CRM activity that requires the live provider; one completed-interaction email — [ADR-0052](./DECISIONS.md#adr-0052--a-supervised-tenant-is-resolved-from-its-own-number-post-call-events-are-correlated-through-the-webhook-ledger-crm-and-email-evidence-require-the-live-provider) | 🟠 Repository implementation in review; activation, provider configuration, deployment, and any live call remain gated |
