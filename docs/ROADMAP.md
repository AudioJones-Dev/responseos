...
| **v0.3 Live Call Demo Slice** | Dedicated ResponseOS demo number; inbound lead call first, outbound request-call second; demo-only tenant; webhook signature validation before mutation; consent, rate limits, spend cap, and kill switch | 🟡 Planned / scoped ([ADR-0039](./DECISIONS.md#adr-0039--v03-live-call-demo-slice-dedicated-demo-number-inbound-first-outbound-second)) |
...
- **Live Call Demo Slice (ADR-0039):** a bounded public demo using a dedicated ResponseOS demo number, demo-only tenant, inbound call first, outbound request-call second. This is the first concrete live-provider experience target, but it still requires separate implementation PRs for provider setup, signature validation, tenant-isolated persistence, consent/rate-limit controls, spend cap, and kill switch behavior.
