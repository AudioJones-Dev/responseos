...
- **Escalation-triggered CRM follow-up task for outcomes that are not qualified (requires an ADR-0047 amendment):** today the supervised-pilot closure exports a follow-up task only on a qualified outcome — `QUALIFIED_FREE_EVALUATION` or `QUALIFIED_QUOTE_REVIEW` — matching ADR-0047 and `runCrmSyncForCall`. The deferred scope is a task on escalations that are *not* qualified: an unavailable or failed live transfer (FRL policy §11), and the project-review outcomes `BUILDER_GC_PROJECT_REVIEW` and `COMMERCIAL_PROJECT_REVIEW`. These are real leads that presently reach a human by ledger and notification rather than as a CRM task. Widening the trigger expands ratified closure behaviour, so **ADR-0047 must be amended before implementation** — a documentation change alone must not do it. Scope defined in [`ops/client-delivery/FRL_INBOUND_INTAKE_ROUTING_POLICY.md`](./ops/client-delivery/FRL_INBOUND_INTAKE_ROUTING_POLICY.md) §16.
