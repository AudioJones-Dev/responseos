...
| **Internal Reference Tenant Foundation** | Explicit account classification; Tyrone internal-demo tenant; separate public-demo sandbox; reusable agent profiles and opportunities; deterministic mock fixtures; commercial-report exclusions | 🟡 In review / mock-only ([ADR-0047](./DECISIONS.md#adr-0047--internal-reference-tenants-use-the-production-account-model-with-explicit-classification)) |
...
The **internal reference tenant foundation** is an additive, mock-only branch under review. It classifies Tyrone's tenant as `internal_demo`, keeps the anonymous `responseos-demo` sandbox separate, and excludes non-customer accounts from commercial reporting. It does not authorize or implement the v0.3 live-provider path or the v0.4 knowledge layer.
