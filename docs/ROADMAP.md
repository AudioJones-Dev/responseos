...
| **Persisted prospect sandbox** | Dedicated fictional `sandbox` tenant, lifecycle read, reset command, and database-backed walkthrough with static fallback — [brief](./product/responseos-persisted-sandbox-demo.md) | 🟡 Implemented locally for review (no live provider or deploy) |
