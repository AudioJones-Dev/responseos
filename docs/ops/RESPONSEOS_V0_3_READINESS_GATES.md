...
**Current state:** PR #128 is merged. Protected run `32530229689` proved the staging Environment `NEON_API_KEY` can read the canonical Neon project/branch/endpoint/database, verified the pooled/direct GitHub pair, synchronized the Vercel Preview Sensitive database variables, and stored a current revision-bound attestation. Final certification stopped only because the configuration lane invoked `vercel link`, whose CLI user lookup returned `User not found. (404)`. REST-only final certification is under review; no migration or deployment ran. A successful rerun, staging deployment/migration, authenticated tenant smoke, and rollback exercise remain uncleared.
