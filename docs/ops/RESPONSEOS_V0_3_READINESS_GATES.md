...
**Current state:** Protected run `32538420957` passed REST-only generic Preview certification. The empty governed Vercel custom environment `staging` (`env_uX6Qp8F6w9aBgx2ikH3BiREB8aHH`) exists with no branch matcher, domain, alias, or deployment. Migration of the nine certified variable scopes plus version 2 environment-bound recertification is under review; no Vercel configuration mutation, Neon migration, or deployment is authorized by that review. The one-time scope migration run, staging deployment/migration, authenticated tenant smoke, and rollback exercise remain uncleared.
