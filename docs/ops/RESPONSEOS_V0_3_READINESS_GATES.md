...
**Current state:** Protected run `32538420957` passed REST-only generic Preview certification. The empty governed Vercel custom environment `staging` (`env_uX6Qp8F6w9aBgx2ikH3BiREB8aHH`) exists with no branch matcher, domain, alias, or deployment. The scope-migration workflow now fail-closes before mutation unless the exact checked-out Deploy Staging workflow verifies the custom-environment ID and targets slug `staging`; current generic Preview deployment controls intentionally fail that check. Deployment-workflow hardening, the one-time scope migration/recertification run, staging deployment/migration, authenticated tenant smoke, and rollback exercise remain uncleared.
