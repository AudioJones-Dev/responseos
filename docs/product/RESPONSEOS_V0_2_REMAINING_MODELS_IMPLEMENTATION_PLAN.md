...
|---|---|---|---|
| **Q1** | `provider_connections.credentials_encrypted` — store as Postgres `Bytea` or `Json` of encrypted blobs? Which encryption mechanism — application-layer (env-keyed AES-GCM via a service worker), per-tenant KMS key, or Supabase-managed Vault? | `RESPONSEOS_DATA_MODEL.md` §4.4 says "encrypted at rest, decrypted at request time" but does not pick a mechanism. ADR-0006 (R2/S3) doesn't apply. No existing ADR covers per-tenant key isolation. **Resolved by [ADR-0020](../DECISIONS.md#adr-0020--provider-credential-encryption-app-layer-env-managed-key-opaque-ciphertext-v02-substrate):** `Bytes` column with envelope framing (`[version \| algorithm \| nonce]` + AES-256-GCM ciphertext); env-managed key contract for v0.2 (placeholder `RESPONSEOS_PROVIDER_KEY`); mock-fallback to a redacted sentinel when the key is absent; KMS / Vault / per-tenant key isolation deferred to a future ADR. | 31A |
