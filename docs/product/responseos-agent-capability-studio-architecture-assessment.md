...
| Latest ADR | VERIFIED (re-verified 2026-09-14) | At the `ec4eb1d` snapshot, `ADR-0053` was highest and this decision was drafted as ADR-0054. `master` has since landed 0055 (#175) and 0056 (#169); open PRs hold 0054 (#176) and 0057 (#177, FRL). | This decision is **ADR-0058** per the owner-settled namespace of 2026-09-14: merged ADRs keep their numbers, open PRs absorb the moves, and FRL (0057) precedes this generalized layer (0058). |
...
1. **ADR-0058** — where the capability definition of record lives. *Decision only, no code.*
...
**Yes — ADR-0058 is required, and it is the first increment.**
...
| # | Question | Answer |
|---|---|---|
| 1 | Which layer (§8)? | Authoring/governance layer above execution, below product surface. Layer placement itself needs ADR-0058. |
...
Per the brief's own gate: a consequential unresolved decision **does** exist (ADR-0058, plus the
...
**Recommendation: do not proceed into implementation beyond increment 1 (ADR-0058) without
...
Added after the operator accepted the *direction* ADR-0058 proposes and revised the implementation
strategy. This section supersedes §13's sequence. Sections 1–16 above remain the
current-state assessment that justified it.

**Accepting a direction is not ratifying the decision.** ADR-0058 remains `Proposed`
...
## A. Evidence supporting the ADR-0058 decision
...
**Database-of-record with a draft UI.** Rejected *for now*, not forever (ADR-0058 q11/q12).
...
`new_lead`, …) and `LeadEventType` already exist. Inventing a third is the collision ADR-0058
decision 9 names.
...
**Naming resolution (ADR-0058 decision 9), settled here.** **"Capability"** is the term for a
...
stays explicitly open (ADR-0058 decision 10).

## D. Migration path toward database-backed authoring

Additive and reversible. (1) Evidence accumulates against ADR-0058 q11. (2) A superseding ADR
...
| `componentVersions[]` | — | — | **NO** — containment already pins them (ADR-0058 q7) |
...
not what starts a capability. Widening either to fit would *be* the collision ADR-0058 decision 9
...
**Why each new abstraction exists** (ADR-0058 decision 8):
...
- `validateCapabilityGovernance` — one pure function. Justified because ADR-0058 decision 5's
...
which contradicted ADR-0058's own `Proposed` status and the operator's hold on
Increment 2. Raised by Codex on PR #174.

## J. Builder-UI gate

Not built until **at least two** of ADR-0058 q11's conditions are demonstrated. Until then, Git
