...
| CRM SoR | `ProviderConnectionProvider`: hubspot, … | `hubspot/`, `ghl/` (stubs) | `HUBSPOT_ACCESS_TOKEN`, `GHL_API_KEY` |
...
**Implemented today:** only `lib/providers/voice/` (`index.ts` + `mock.ts` + `types.ts` — the single real, mock-only CAL slice) and `lib/providers/encryption/` (AES-256-GCM at the adapter boundary, ADR-0020). `ProviderConnection` stores `credentials_encrypted` (Bytes) under a unique `(account_id, provider)` with tenant scoping. **Every other adapter is an empty `.gitkeep` stub.** This is correct for mock-first/v0.2 — it means v0.3 starts from near-zero adapter code.

## 3. Research-vs-ADR reconciliation

The research README ([#71](../research/communications-stack/README.md)) is **correctly labeled a snapshot** and explicitly defers to `DECISIONS.md` as canon, so it does **not** itself conflict. Summary:

| Capability | ADR canon | Research README | `BUILD_SOURCE.md` |
|---|---|---|---|
| Carrier | Telnyx primary, Twilio failover (ADR-0031) | Telnyx primary, Twilio fallback | "Twilio edge" — **stale**, ADR-0031 says update to Telnyx |
| AI orchestration | Vapi primary, Retell secondary (ADR-0032) | "Vapi *or* Retell, deferred" | (Grok-stack framing) — **needs reconciliation** |
| LLM brain | OPEN (ADR-0032 §3) | n/a | implies Grok — **unresolved** |
| CRM SoR | HubSpot default, client-overridable (ADR-0033) | n/a | varies |
| iMessage | none | **Sendblue (optional)** | none — new candidate, no ADR |
| Abstraction | CAL (ADR-0001) | CAL internal | — |
...
8. **Stale docs cleanup** — `BUILD_SOURCE.md` ("Twilio edge"), and `SECURITY.md` ("Standard mode runs on Twilio + Retell + Supabase + Vercel" + the webhook table missing Telnyx/Vapi rows) predate ADR-0031/0032 and need updating.
...
2. **All adapters are `.gitkeep` stubs** except `voice/` (mock) and `encryption/`. No `CarrierProvider` / `VoiceAgentProvider` / `CrmProvider` interfaces exist in code — only the `voice/` types.
