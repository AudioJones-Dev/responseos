...
3. **`lib/automations/`** is an empty directory that `architecture.md` describes as a populated module. **`lib/notifications/`** now holds exactly one notification — the supervised tenant's completed-interaction email (ADR-0052) — and is not the general notification engine `architecture.md` describes. **`lib/config/`** holds only per-client operating-configuration skeletons under `lib/config/clients/`; no typed env/runtime config loader exists there.
