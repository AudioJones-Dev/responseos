...
2. **`consent_records` does not exist.** It is specified in `docs/data-schema.md` and absent from all 22 Prisma models. That is a migration with tenant-scoped accessor, mock parity, and isolation tests — not a configuration step. Per **ADR-0055** the migration must carry immutable consent *events* as the record of authority, each naming its source channel, with per-contact state derived from them; a per-contact-only model does not satisfy it.
...
4. Consent semantics, jurisdiction logic, and cross-channel consent are entirely uncovered by Vapi. **Amended 2026-09-13 by ADR-0055:** the earlier "per-call grant → per-contact state" shorthand understated what has to be built. The record of authority is an **immutable event** carrying the disclosure presented, the grant or withdrawal, the timestamp, the jurisdiction basis and the source channel — so SMS, web-form and outbound-campaign consent are the same record, which is what the cross-channel item below requires; for a recording or persisted transcript the event must also be linked to that call and artifact class. per-contact state is a *derived summary*, not the evidence. Recording and transcript persistence are authorised independently, and absence of an event is refusal. Building only the per-contact row would satisfy this item's letter while leaving no way to establish which grant authorised a given retained artifact — which is what FRL policy G-12 requires before activation.
