...
2. **`consent_records` does not exist.** It is specified in `docs/data-schema.md` and absent from all 22 Prisma models. That is a migration with tenant-scoped accessor, mock parity, and isolation tests — not a configuration step. Per **ADR-0055** the migration must carry per-call immutable consent *events* as the record of authority, with per-contact state derived from them; a per-contact-only model does not satisfy it.
