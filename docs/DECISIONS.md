...
## ADR-0054 — The capability definition of record lives in Git; the database holds only the release assignment

**Status.** Proposed · 2026-09-13 · pending operator ratification. Extends **ADR-0001** (mock-first), **ADR-0017** (orchestration placement), and **ADR-0051** (execution modes). **Authorizes no implementation** — no schema change, migration, table, authoring surface, engine, or provider activation. It decides *where a capability definition lives* so that the question is settled before code exists, not after. **Does not authorize v0.3** (doctrine D-1 stays open) and moves no item in doctrine §22.

**Context.** An operator brief proposes an **Agent Capability Studio**: an internal surface for authoring, validating, simulating, versioning, publishing, and pinning governed revenue-agent behaviours. The repository-grounded assessment is [`product/responseos-agent-capability-studio-architecture-assessment.md`](./product/responseos-agent-capability-studio-architecture-assessment.md). Three findings from that assessment set up this decision.

First, **much of the governance machinery the brief specifies already exists as working code**, not as documentation. `lib/agentExecution/policy.ts` implements explicit `allowedTools` allowlists and a `resolveExecutionPolicy` that fails closed to the demo lane; `EXECUTION_MODE_ACTIVATION_GATES` binds each mode to a *named* gate rather than a boolean, precisely so an approval issued for one gate cannot unlock a mode behind another. `lib/prospectBootstrap/template.ts` holds `PROSPECT_RECEPTIONIST_TEMPLATE` with a SHA-256 checksum and a preflight validator. `BootstrapPromotion` is a hash-pinned publication manifest with a forbidden-field scanner. The brief asks for these primitives; they ship.

Second, **the repository has already answered a version of this question and answered it "Git."** `lib/agentExecution/policy.ts` records that `lib/prospectBootstrap/service.ts` compares a stored `AgentProfile.system_policy_json` *byte-identically* against a frozen Git constant, "so its shape and values must not drift." The definition of record is the Git constant. The database row is the assignment of that definition to a tenant. That pattern was chosen deliberately and is load-bearing today.

Third, **there is no generic step-execution engine.** No step walker, condition evaluator, or suspend/resume primitive exists anywhere in `lib/`. What does exist is *hardcoded* orchestration — `runCrmSyncForCall` (`lib/crm/syncFinalizedCall.ts`) sequences provider calls with idempotent state transitions on `CrmSyncOperation`, and `answerProfessionalQuestion` (`lib/professional/receptionist.ts`) classifies a turn and then branches deterministically through `applyPolicy`. ADR-0017's "core RECOVER orchestration lives in code" is therefore accurate. The gap is not that orchestration is missing; it is that orchestration is written per use case and its execution is not recorded against any capability version. Relatedly, `WorkflowRun` has **no production writer** — `recordWorkflowRun` and `finalizeWorkflowRun` are called only by integration tests and seed scripts.

An earlier draft of this analysis asserted that ADR-0017 was `DOCUMENTED_ONLY` on the strength of the missing `WorkflowRun` writer. That conflated "nothing records runs" with "nothing orchestrates," and it was wrong. Recording the correction here because the distinction is the whole reason this ADR recommends generalizing an existing pipeline rather than commissioning an engine.

**The decision this forces.** A capability definition can live in one of two places, and the choice is expensive to reverse once an authoring surface exists:

- **Git of record** — the definition is a typed, checksummed module; the database stores which tenant runs which checksum.
- **Database of record** — the definition is a draft/version row set, authored through a UI, with publication and immutability enforced in application code.

These are not stylistic variants. They put the hardest invariant in the capability model — *a published version is immutable and an execution must name an exact one* — in different places. Under Git, immutability is structural: a checksummed committed artifact cannot change without changing its hash, and `AGENTS.md` already requires independent review and a human merge to alter it. Under a database, immutability must be *built and defended*: update guards, tests proving no mutation path, and a new in-app publication authority that partially duplicates the merge gate the governance kernel already reserves to the human.

**Decision.**

1. **The capability definition of record is a Git-authored, typed, checksummed module.** It generalizes the existing `PROSPECT_RECEPTIONIST_TEMPLATE` pattern rather than introducing a parallel one. The checksum over the frozen capability object *is* the version identifier; because it covers the whole object graph — prompt text, scripts, step definitions, tool policy, validators — pinning one hash pins every dependency transitively. This yields the brief's "deterministic manifest at publication" structurally, rather than by assembling and hashing one at publish time.

2. **The database holds the release assignment and nothing else.** One new table records which account runs which capability slug at which version checksum, under which execution mode, approved by whom and when. Runtime resolves that exact checksum or **fails closed**. `latest`, `current`, `head`, and `default` are never resolvable for behaviour-critical components. Restoration is a new assignment row pointing at a prior checksum, never a mutation; deprecation is assignment status, and the artifact is never deleted, so any historical execution stays reconstructable.

3. **Publication approval is the existing human merge.** No new in-app publication authority is created. This is not merely economical — the governance kernel and `AGENTS.md` reserve merge to the human and prohibit agents from merging, and an in-app "publish" button that changed live behaviour without a merge would route around that control. Activation of a published capability for a tenant remains the existing operator approval-route pattern (`app/api/admin/prospect-bootstraps/[id]/approve|activate`).

4. **Per-capability tool policy intersects the mode policy; it never replaces it.** Effective permissions are the intersection of the capability's declared `allowedTools` with the resolved `ExecutionPolicy.allowedTools` for the tenant's mode. Intersection is structurally incapable of escalation, so an authored capability cannot grant itself a tool the tenant's mode withholds. The frozen `ExecutionPolicy` object shapes must not drift, per the stored-policy comparison in `lib/prospectBootstrap/service.ts`.

5. **No authoring UI, step engine, component library, semantic-diff surface, or AI-assisted authoring is authorized.** Each is premature while exactly one capability template exists and doctrine §22 item 7 — reconcile one pilot manually, from inbound event to collected outcome — has not happened. `AGENTS.md` scope discipline applies directly: three similar lines beat a premature abstraction.

6. **This decision is scheduled for revisit, not settled forever.** It is reconsidered when **both** hold: at least three capabilities exist as checksummed modules, and doctrine §22 item 7 is complete. At that point the marginal cost of the next capability is measurable rather than assumed, and a database-of-record model can be argued from evidence. Recording the revisit trigger prevents this ADR from ossifying into a permanent veto on the Studio.

7. **Naming is reconciled before any capability module is written.** `app/(admin)/admin/playbooks/page.tsx` is a hardcoded three-element array behind a "activates in v0.3" banner, and `Automation` is an existing model with its own trigger enum. "Capability", "playbook", "template", and "automation" must not accumulate as four names for overlapping ideas.

**What this does not decide.** Whether to build a step-execution engine at all, and if so which runtime lane it targets — the assessment verified that **runtime ownership is currently split**: prospect-demo turns are provider-hosted (a prompt plus `dynamicVariables` pushed at preflight), while professional-lane turns execute in-process through `answerProfessionalQuestion`. An engine must target one lane first or it will be built twice. That is a separate decision requiring its own ADR, and nothing here authorizes starting it.

**§21 checklist.** **Layer:** an authoring and governance layer above execution, below the product surface; this ADR places it and authorizes nothing in it. **Built/integrated/deferred:** mostly deferred — definition format and pinning are built, engine and authoring surface deferred. **Live pilot path:** no improvement, and honestly so — both flagship capabilities in the brief are gate-blocked from live execution today (outbound is `false` below `MANAGED_AUTONOMY`; scheduling needs `v0.3-live-communications`, doctrine D-1). **Evidence:** preserved and improved in principle — version-pinned lineage on `WorkflowRun` would make "which behaviour produced this outcome" answerable — but see consequences on the missing writer. **Verified outcomes:** supports, does not itself verify. **Proprietary learning:** yes, and this is the brief's strongest argument — a governed library of service-business revenue procedures compounds. **Commodity to buy:** partly *already owned* — Git plus the PR process supply immutability, diff, review, approval, and audit. Runtime pinning, permission-broadening detection, and simulation traces are genuinely new surface. **Duplicates CRM/FSM/telecom/workflow-platform:** no, provided step types stay domain-shaped; the brief's own non-goals bind here. **Vendor lock-in:** reduced — behaviour is pinned in our artifacts rather than a provider console. **Tenant isolation:** preserved; `account_id` derives from the authenticated server session and any new table joins the tenant-isolation matrix test. **Attribution ambiguity:** reduced. **Public claims (§20):** the live risk. No provenance claim may be made while `WorkflowRun` has no production writer; evidence capture is real (`Call`, `CallTranscript`, `QaLog` have verified writers) but is not attributable to a capability version. **New human-approval control:** none under this decision — that is decision 3's point; a database-of-record model *would* have required one. **Compliance exposure:** slightly increased in the abstract, since authored behaviour must not bypass disclosure or prohibited-advice rules; mitigated structurally by decision 4's intersection. **Required now:** the Studio is not; *this decision* is, because it is cheap now and expensive after an authoring surface exists.

**Consequences.** The repository keeps one model for governed artifacts instead of two, and the `AgentProfile.system_policy_json`-versus-frozen-constant pattern is extended rather than contradicted. Immutability costs nothing to enforce, and the human merge gate remains the single publication authority, satisfying the governance kernel without a parallel in-app control.

The cost is real and should not be softened: **capability authoring remains an engineering activity.** An operator who cannot open a pull request cannot author a capability. `UserRole` models a non-engineer `operator` role, so the population may exist — but whether staff in that role would author capabilities is unverified and is an operator question, not one the repository can answer. If the answer is that they must, decision 6's revisit arrives sooner and the database-of-record option gets stronger. A read-only operator view listing capabilities, pinned versions, validation status, and last simulation result is compatible with this ADR and would give `admin/playbooks` real content in place of its stub.

Finally, this ADR makes a gap visible rather than closing it: with no production writer for `WorkflowRun`, version-pinned execution provenance is designed and not operational. Until a writer exists on the post-call path, no document, dashboard, changelog entry, or customer-facing surface may describe ResponseOS as tracing outcomes to capability versions. Doctrine §20 governs that language.
