...
**Amendment (2026-09-11) — recording is tenant-configurable, and the policy table still says `false`.** Operator decision. Decision 1 held recording `false` at every tier. It still does: no entry in `EXECUTION_MODE_POLICIES` changes, and the demo lane can never record. What changes is that a *tenant* may turn it on for itself, and only by satisfying all three of the following at once.

1. The tenant's agent profile declares a supervised mode whose stored policy is byte-identical to the table entry.
2. The deployment lane authorises that mode's activation gate.
3. The tenant's approved operating configuration carries a `policy.consent` fact whose `recording.enabled` is `true`, with the disclosure text, the continuation statement, and a retention period.

`resolveTenantExecutionPolicy()` in `lib/agentExecution/tenantPolicy.ts` returns a new frozen policy with `recordingEnabled: true` and the tenant's disclosure as `requiredDisclosure`; it never mutates the shared table. Any of the three missing leaves recording off, and an unrecognised or tampered stored policy degrades the whole tenant to the demo lane rather than throwing.

**A script is not evidence.** An instruction telling the agent to speak a disclosure proves nothing about how the provider is configured, and this repository makes no legal-compliance claim from either. Activation therefore requires a signed provider attestation whose attested recording posture — at the assistant *and* at the number, which carry separate switches — equals the tenant's resolved posture, checked by `validateSupervisedAssistantPreflight()`. A mismatch in either direction fails activation.

**Refusal is recorded, not enforced by us.** `Call.recording_consent` stores `continued`, `refused`, or `unknown`. On refusal the agent acknowledges, offers a person or a callback, and minimises what it collects. Stopping an in-progress recording is **not implemented**: it needs a provider call-control command and an account-wide provider API key that the deployment lane forbids. Recording also begins when the call is answered, which is before the disclosure is spoken. Both are stated rather than claimed away.

---

## ADR-0052 — A supervised tenant is resolved from its own number; post-call events are correlated through the webhook ledger; CRM and email evidence require the live provider

**Status:** Accepted (2026-09-11). **Implements** ADR-0051 decisions 1 and 3 for one live tenant lane. **Extends** ADR-0047 (legacy evidence chain) and ADR-0048 (prospect bootstrap); supersedes neither. No tenant is activated by this ADR.

**Context.** ADR-0051 made supervision expressible but nothing read it: `resolveExecutionPolicy()` and `evaluateOperatingConfiguration()` had no runtime callers, no write path produced an `operator_configured` fact, and live call handling ran in one lane keyed to a single demo number held in an environment variable. Meanwhile the provider's documented post-call event turned out not to carry the called number at all, which the existing route treats as an unroutable event.

**Decision.**

1. **A number identifies a tenant.** `TelephonyNumber` plus `TelephonyNumberAssignment` own the binding, as they already do for demo bootstraps. A supervised assignment is one with **no bootstrap**, so the two lanes cannot resolve to each other while sharing the table; `bootstrap_id` becomes nullable and the prospect resolver returns null for a bootstrap-less assignment. A number is never hard-coded in application logic, and supervised traffic never routes through `RESPONSEOS_DEMO_PHONE_E164`.

2. **The tenant's mode comes from its agent profile, and only restricts.** `AgentProfile.system_policy_json` must be byte-identical to that mode's entry in `EXECUTION_MODE_POLICIES`; the deployment lane must authorise the mode's gate, named in `RESPONSEOS_AUTHORIZED_EXECUTION_GATES`. Anything unrecognised, tampered, or ungated degrades the tenant to the demo lane rather than throwing. Recording is the one capability a tenant can add, under the ADR-0051 amendment above.

3. **Destination-less provider events correlate through the webhook ledger.** `WebhookEvent` gains `provider_call_id` and `agent_target`. The initialization and conversation-end events carry the called number and anchor it against the call id; an insight event carrying only call-control identifiers recovers its tenant from those rows. Only a number the provider itself put on an event may become a correlation anchor, so a correlated value cannot re-anchor itself. An event that correlates to nothing is rejected as `awaiting_call_correlation`, distinct from a genuinely unknown destination.

4. **Structured call detail is read by parsing, not by provider identifiers.** Insight results arrive as strings, JSON-encoded when the insight template declares a schema. The extractor parses each result and takes the first that validates, treats a prose result as the summary, and maps only recognised classification values — an unrecognised label is recorded as absent rather than guessed. No provider insight id is stored in tenant configuration. The undocumented `payload.summary` / `payload.qualification` readers remain for the legacy lane.

5. **Transcripts arrive by push.** `send_message_history_updates` delivers conversation turns to the signed webhook. Pulling them from the provider's messages API would require an account-wide provider API key in the runtime, which the live-demo lane forbids and which would widen blast radius far beyond reading one transcript. A later event with fewer turns never truncates a stored transcript.

6. **A live tenant is never told a mock write succeeded.** A supervised CRM sync records the operation against `hubspot`, so the existing stickiness check fails the attempt as `live_provider_disabled` when the live adapter is not active. The call activity carries one structured ResponseOS summary block on the existing activity object — **no note object is created** — and neither a transcript nor a recording URL is sent. A quote request earns a follow-up task even when the score did not reach qualified.

7. **Notification is a durable row, then a dispatch.** A `Notification` row is written first, keyed by a dedupe key, with its subject and body frozen at first dispatch; the provider send follows, then `sent` or `failed` with a retryable error code and an operator retry route. Retries resend the stored text verbatim under the same idempotency key, because rebuilding the body would change the payload behind an unchanged key. The body carries operational fields only: never the transcript, the recording, the provider payload, credentials, or CRM history. A failure never fails the call. This is one notification for one event, not a notification platform.

8. **Client operational data enters only through the operator write path.** One operator-only route configures the account, agent profile, approved configuration snapshot, number, and activation, with a dry run. Escalation contacts, notification recipients, and dedicated numbers live in the database and appear in no source file, fixture, audit metadata, dynamic variable, or public surface. Audit entries record configured key names and hashes, never values.

9. **The legacy evergreen demo path is unchanged** and stays as a documented compatibility shim through pilot certification. A cleanup recommendation follows certification rather than preceding it.

**Consequences.** One deployment can now serve a supervised customer tenant and the demo lanes at once, distinguished by number rather than by environment flag. Three provider prerequisites sit outside this repository and block certification, not merge: a verified email sending domain, the attestation signing key, and a stable webhook URL for the live-demo lane, which deploys to a changing preview URL. The provider event envelope, the event ordering, and whether message history arrives cumulatively are **assumptions confirmed by the first controlled call**; the raw signed body is retained for a supervised tenant so that call can settle them from evidence.
