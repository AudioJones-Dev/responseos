...
Calls originating inside a qualification interval remain permanently ineligible for CRM or email dispatch, including after later normal activation. Assistant initialization pins the resolved assignment identity on `CallCaptureSession`; business-effect authorization uses that durable identity before legacy timestamp inference, so missing or delayed provider timing cannot reclassify a qualification call. Ending qualification clears number exclusivity and returns the local inventory number to available without deleting evidence. Final activation still requires the signed provider attestation, verified consent/capture fields, exact execution gate, enabled profile, and existing readiness checks. Migration 0016 adds the explicit enum value and a nullable capture-session assignment pin; it changes no existing assignment state.

### Ratified Call Control evidence amendment — 2026-09-16

The observed direct TeXML/no-code assistant path cannot establish the required consent interval and is rejected for FRL qualification. Qualification uses Telnyx Voice API Call Control: deterministic disclosure playback completes before a DTMF gather, and the AI assistant is not started until an authenticated operator records the witnessed affirmative response against the pinned capture session. Refusal, timeout, an unsupported digit, or conflicting state starts no AI or transcription. TeXML form callbacks remain a separate provider contract and are never parsed as JSON AI evidence.

Telnyx support issue `#669837` is the provider evidence for the narrow boundary amendment. Telnyx does not emit signed effective-time webhooks for `ai_assistant_start` or `ai_assistant_stop`; the synchronous authenticated command response is its acknowledgement. With recording disabled and before AI, transcription, gather, streaming or fork commands, Telnyx reports that it retains only ordinary call-signaling metadata. A valid start boundary therefore requires the witnessed grant, immutable `CallConsentEvent`, committed idempotent command intent, authenticated successful Telnyx response with provider response time and conversation ID, and the first signed downstream history event bound to the same capture generation. No one component, prompt text, transcript phrase, local receipt time or unsigned metadata is sufficient.

Withdrawal first commits the immutable consent event and closes ResponseOS content admission. It then records an idempotent stop intent, obtains the authenticated Telnyx response, waits for signed conversation-ended evidence, and rejects every later protected-content event. Normal conversation end uses the signed provider occurrence time. Qualification still does not satisfy provider attestation or activation readiness.

Recording and Telnyx data retention remain disabled. Telnyx Insight Groups cannot produce the required result under the approved no-retention posture and are removed from the canonical qualification path. Signed admitted cumulative history produces hashed immutable transcript revisions. A replaceable ResponseOS post-call analysis adapter produces a schema/version/model-labelled interpretation whose uncertainty remains explicit; inference is not source evidence. Migration 0017 adds only the Call Control session fields, idempotent command ledger, transcript revisions and post-call analysis provenance. Qualification-origin CRM, email and autonomous follow-up remain permanently forbidden.
