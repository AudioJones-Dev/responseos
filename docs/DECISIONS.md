...
1. **Supervision is a mode-indexed execution policy, not a tenant column and not only a deploy lane.** `lib/agentExecution/policy.ts` declares `ExecutionMode` (`PROSPECT_DEMO`, `SUPERVISED_PILOT`, `PRODUCTION_SUPERVISED`, `MANAGED_AUTONOMY`) and a policy table of identical shape per mode, reusing the shipped checksum-pinned policy mechanism.
   ...
   - `resolveExecutionPolicy()` **fails closed**: an unrecognised mode, or a gated mode without explicit authorisation, resolves to the most restrictive policy rather than throwing.
