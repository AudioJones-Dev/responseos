...
3. **Callers fail closed.** An error envelope means the stored policy is unknown, and the strict `DEFAULT_AGENT_PROFILE_POLICY` — no shareable assets, compensation and references escalated — is the safe reading. Falling back to the fixtures on a failed read would reintroduce exactly the drift this removes. A tenant whose profiles are all disabled resolves to the same strict default, so disabling is a working revocation rather than a no-op.
