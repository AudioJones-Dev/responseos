...
Structurally, by containment: a checksum computed over a frozen object graph transitively pins every component that graph *contains*. This is how `PROSPECT_RECEPTIONIST_TEMPLATE_CHECKSUM` behaves over the template's prompt text, dynamic variables, and tool list.
