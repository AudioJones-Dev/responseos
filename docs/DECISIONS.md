...
3. Acquired text is untrusted evidence, never a prompt. Schema-bounded facts retain URL, content hash, retrieval time, evidence excerpt, confidence, verification state, and conflicts. Only `OPERATOR_APPROVED_FOR_DEMO` or `OWNER_CONFIRMED` facts compile into an immutable, checksummed `BusinessMemorySnapshot`; unknowns and conflicts compile to refusal/fallback behavior.
