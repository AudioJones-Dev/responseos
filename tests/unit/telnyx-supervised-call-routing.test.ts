...
describe("supervised Telnyx call lane", () => {
  ...
  test("captures identity, requires the live CRM provider, and sends one notification", async () => {
    ...
    expect(mocks.runCrmSyncForCall).toHaveBeenCalledWith(
      expect.objectContaining({ requireLiveProvider: true, structuredActivity: true, callId: "call-1" }),
    );
    expect(mocks.dispatchCompletedInteractionNotification).toHaveBeenCalledWith(
      expect.objectContaining({ accountId: "account-supervised", callId: "call-1", requireLiveProvider: true }),
    );
