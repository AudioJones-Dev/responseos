...
describe("tenant execution policy", () => {
  ...
  test("turns recording on only from the tenant's approved consent configuration", () => {
    ...
    expect(resolved.policy.recordingEnabled).toBe(true);
    expect(resolved.recordingSource).toBe("tenant_consent_configuration");
    expect(resolved.policy.requiredDisclosure).toContain("being recorded");
...
describe("supervised agent context", () => {
  ...
  test("speaks the tenant's recording disclosure once recording is configured", () => {
    const memory = snapshot([
      ...ENTRIES.filter((entry) => entry.key !== "policy.consent"),
      { key: "policy.consent", value: CONSENT_RECORDING_ON },
    ]);
    ...
    expect(context.recording_enabled).toBe("true");
    expect(context.recording_disclosure).toContain("being recorded");
