...
export function ConsentControls({ id, callReference }: { id: string; callReference: string }) {
  ...
  async function record(action: "grant" | "refuse" | "withdraw") {
    setBusy(true);
    try {
      const response = await fetch(`/api/admin/call-capture/${id}/consent`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, disclosureRef: disclosure, evidenceRef: evidence, eventKey: crypto.randomUUID() }) });
  ...
  return <details className="my-3 rounded border p-4">
    ...
    <div className="flex gap-3">{(["grant", "refuse", "withdraw"] as const).map((action) => <button key={action} className="rounded border px-3 py-2" disabled={busy || !disclosure.trim() || !evidence.trim()} onClick={() => record(action)}>Record {action}</button>)}</div>
