...
- **`.github/workflows/dashboard.yml`** never writes directly to `master`.
  It refreshes the dedicated `automation/dashboard-sync` branch and opens or
  updates a pull request for human review. Repository Actions settings must
  allow `GITHUB_TOKEN` to create pull requests before that automation can
  activate. A maintainer must also approve the workflow runs generated by each
