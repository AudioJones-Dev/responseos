...
- **Live provider integrations are still gated to v0.3.** No Twilio / Retell / Vapi / Stripe / GHL / HubSpot calls run against real accounts from this repo. No production deploys, except the public read-only demo surface (ADR-0053).
...
- No production deploys from this repo (no Vercel, no AWS) until v0.3 readiness gates clear, except the public read-only demo surface (ADR-0053) — the marketing pages and `/demo/receptionist`, on mock adapters with no provider credentials. Automatic git deploys stay disabled.
